...
import { ele_images, ele_mo_images } from "@/elephantSchoolImages";

const ElephantSchoolComponent = () => {
  const [position, setScroll] = useState(0);
  const [index, setIndex] = useState(-1);
  const [indexMo, setIndexMo] = useState(-1);

  const currentImage = ele_images[index];
  const nextIndex = (index + 1) % ele_images.length;
  const nextImage = ele_images[nextIndex] || currentImage;
  const prevIndex = (index + ele_images.length - 1) % ele_images.length;
  const prevImage = ele_images[prevIndex] || currentImage;

  const mo_currentImage = ele_mo_images[indexMo];
  const mo_nextIndex = (indexMo + 1) % ele_mo_images.length;
  const mo_nextImage = ele_mo_images[mo_nextIndex] || mo_currentImage;
  const mo_prevIndex =
    (indexMo + ele_mo_images.length - 1) % ele_mo_images.length;
  const mo_prevImage = ele_mo_images[mo_prevIndex] || mo_currentImage;
...
export default ElephantSchoolComponent;
